function [LL,SystemMatrices_end,StateSpaceP_new,SystemMatrices_end_Collect]=funzioneNowcast_IS(X,r,p,s,nmaxit,isSink,M,learnRate,...
...
S=nan(size(X,2)+1,nmaxit);
P=nan(size(X,2)+1,size(X,2)+1,nmaxit);
C=nan(size(X,2),size(X,2)+1,nmaxit);
R=nan(size(X,2),size(X,2),nmaxit);
Q=nan(size(X,2)+1,size(X,2)+1,nmaxit);
A=nan(size(X,2)+1,size(X,2)+1,nmaxit);

modelSpec.r=r;
modelSpec.p=p;
modelSpec.s=s;
modelSpec.blocks=ones(size(X,2),1);
modelSpec.nM=size(X,2);
M=min(round(M*size(X,1)),20);
LL=nan(nmaxit,1);
...
% initialize classic
[S_init,P_init,C_init,R_init,A_init,Q_init]=initialize_EM_IS(x,modelSpec,isSink);
...
while it<=nmaxit
   ...
   [SystemMatrices_end,loglklhd,x_init]=EMalgorithm_IS(x_end,modelSpec,SystemMatrices_init,dove,isSink);
...
if isSink==1
    ...
    KalmanFilterOutput      =kalman_filter(x_init,SystemMatrices_init);
else
...
StateSpaceP_new.Xhat    =((KalmanSmootherOutput.S_smooth(:,2:end)'*...
        ...
        kron(nanmean(x),ones(T,1));         %estimated X; %estimated X
end
